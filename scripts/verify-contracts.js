...
function createInheritanceGraph(buildArtifacts) {
  ...
  for (const buildArtifact of buildArtifacts) {
    const {
      contractName,
      ast: { nodes },
    } = JSON.parse(shell.cat(buildArtifact));

    const contractDefNode = nodes.find(
...
function isUpgradeableContract(buildArtifact, inheritanceGraph) {
  ...
  try {
    ...
    if (/No path found/gi.test(err.message)) {
      return false;
    } else throw err;
  }
}
...
async function main() {
  ...
  const argv = yargs(process.argv.slice(2)).option("network", {
    default: "development",
    string: true,
  }).argv;
  const { network } = argv;
  ...
  const implAddressesEntries = await Promise.all(
    allBuildArtifacts.map(async (buildArtifact) => {
      ...
      if (network) {
        ...
        return [
          buildArtifact,
          await getProxyImplementation(admin, proxyAddress),
        ];
      }
    })
  );

  const implAddresses = Object.fromEntries(
    implAddressesEntries.filter(Boolean)
  );
  ...
  for (const buildArtifact in implAddresses) {
    ...
    assert.strictEqual(
      !!implAddress,
      isUpgradeableContract(buildArtifact, inheritanceGraph)
    );
  ...
  console.log();
  console.log(
    `Verifying: ${plainContractsToVerify.join(
      " "
    )} ${implContractsToVerify.join(" ")}`
  );
  console.log();

  shell.exec(
    `npx truffle run verify ${plainContractsToVerify.join(
      " "
    )} ${implContractsToVerify.join(" ")} --network ${network}`
  );
