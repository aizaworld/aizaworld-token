...
function backupNetworks(networkId) {
  ...
  shell
    .ShellString(JSON.stringify(savedNetworks, null, "  "))
    .to(`networks/networks-${networkId}.json`);

  console.log(
    `Addresses for network ID ${networkId} have been backed up to networks/networks-${networkId}.json.`
  );
}

function restoreNetworks(networkId, doForce = false) {
  const savedNetworks = JSON.parse(
    shell.cat(`networks/networks-${networkId}.json`)
  );
  ...
  for (const artifactFile in savedNetworks) {
    ...
    if (catRes.code !== 0) {
      console.error(
        `WARNING: UNABLE TO READ FILE ${artifactFile} - have you made sure it exists?`
      );
      continue;
    }
    ...
    if (
      doForce ||
      _.isEqual(
        buildArtifact.networks[networkId],
        savedNetworks[artifactFile]
      ) ||
      ...
      console.error(
        `ERROR: File ${artifactFile} contains data for network ID ${networkId} that differs from backed up data - pass --force to force overwriting any existing data.`
      );
  ...
  console.log(
    `Addresses for network ID ${networkId} have been restored from networks/networks-${networkId}.json to the build artifacts.`
  );
}
...
yargs(process.argv.slice(2))
  .command(
    "backup <networkId>",
    "Backup networks from build artifacts",
    (yargs) => {
  ...
  .command(
    "restore <networkId>",
    "Restore networks to build artifacts",
    (yargs) => {
      yargs
        .check((argv) => Number.isInteger(argv.networkId))
        .option("force", {
          description:
            "Force overwrite existing network data even if it is different",
        })
        .boolean("force");
    },
    (argv) => {
