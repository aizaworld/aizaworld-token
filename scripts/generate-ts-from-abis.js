...
function generateAbiInterfaceTs(contractName, abi) {
  const generateFunction = (fnAbi) => {
    const params = fnAbi.inputs
      .map(
        (paramAbi, i) =>
          `${paramAbi.name || "arg" + i}: ${transformAbiTypeToInputTsType(
            paramAbi.type
          )}`
      )
      .join(", ");
    const returnTypes = fnAbi.outputs.map((outputAbi) =>
      transformAbiTypeToOutputTsType(outputAbi.type)
    );
...
for (const buildArtifact of shell.ls("build/contracts/*.json")) {
  ...
  generationInterfaceOutput += generateAbiInterfaceTs(
    data.contractName,
    data.abi
  );
...
console.log(
  `Generated build/abi-interfaces.ts from ${contracts.size} contracts`
);
